...
aod_dir = "C:/Users/zy125/Box Sync/Postdoc/BABIES/beijing_data/aod"
...
for(year in 2018) {
  ...
  setwd(file.path(aod_dir, year, "H27V05"))
...
st_write(p_geo,"aod275_2018_geo.shp", driver = "ESRI Shapefile")

shanghai_boun<-st_read("test1.shp")

p_clipped<-st_intersection(p_geo, shanghai_boun)

ggplot() + geom_sf(data = p_clipped, colour = "light gray", fill = NA)

mapview(shanghai_boun, zcol='all.mean')
